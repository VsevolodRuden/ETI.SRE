...
$(document).ready(function() {
    accountBillingType = Xrm.Page.getAttribute("eti_accountbillingtype").getValue();
    if (accountBillingType == 964820000) {
        if (Xrm.Page.getAttribute("eti_residentalpricelist") != null)
            priceLevelId = Xrm.Page.getAttribute("eti_residentalpricelist").getValue()[0].id;
    } else if (accountBillingType == 964820001) {
        if (Xrm.Page.getAttribute("eti_commercialpricelist") != null)
            priceLevelId = Xrm.Page.getAttribute("eti_commercialpricelist").getValue()[0].id;
    }
    if (!priceLevelId) {
...
function resetPrice() {
    if (rightTree.select().length) {
        ...
        var data = rightTree.dataItem(row);
        console.log(data);
        if (data.productStructure == 1 && (data.originalRate != data.newRate) && !(data.productStateValue == 964820003 || data.productStateValue == 964820006)) {
            ...
            Xrm.WebApi.updateRecord("eti_accountproduct", data.accountProductId.replace(/[{}]/g, ""), payload).then(
                ...
                function(error) {
                    displayErrorMessage(error.message, "Can't reset Price!");
...
function confirmChanges() {
    if (rightTree.select().length) {
        ...
        if (data.productStateValue == 964820003) {
            Xrm.WebApi.deleteRecord("eti_accountproduct", data.accountProductId.replace(/[{}]/g, "")).then(
                function success(result) {
                    console.log("AccountProduct updated");
                    refreshActiveGrid();
                    refreshPendingGrid();
                ...
                function(error) {
                    displayErrorMessage(error.message, "Can't delete Assignment!");
                    // handle error conditions
                });
        } else {
